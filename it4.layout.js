...
*/
var layout={};
var getContentHeight=function(el){let h=0;
	for(let c in el.childNodes){
		h=h+el.childNodes[c].offsetHeight;
	}
};
layout.Item=function(target,opts,_ui){this.el=false;this.ui=_ui;this.opts=opts||{};this.init();};layout.Item.prototype={
	init:function(){
		if(!this.opts.x){this.opts.x=0}if(!this.opts.y){this.opts.y=0}
		if(!this.opts.w){this.opts.w='auto'}if(!this.opts.h){this.opts.h='auto'}
		let attrs=[];let style='';
		if(!opts.noresize){style=style+'resize:both;overflow:auto;'}
		if(style!=''){attrs.push('style');attrs.push(style);}
		this.el=it4.ins(target,this.opts.tagname,attrs,false,target.firstChild);
...
layout.Area=function(el,opts){this.el=it4.$$(el);this.opts=opts||{};this.items={};this.init();};layout.Area.prototype={
	init:function(){
		if(!opts.cols){opt.cols:12}
		if(!opts.heightstep){opt.heightstep:1}
		if(!this.el.style.display=='flex'){
			console.log('Setting style to flex');
	...
	refitItem:function(item){
		...
		if(item.opts.h=='auto'){
			let hh=getContentHeight(item.el);
			item.el.style.height=hh+'px';
		}
	},
	load:function(items){
...
(function(exports){exports.layout=layout;})(typeof exports === 'undefined'? (ISNODE?this['layout']={}:this['it4']['layout']=layout):exports);
