...
var GraphTable=function(opts,source,target){GraphBase.call(this,'Table',opts,source,target);};GraphTable.prototype={
	redraw:function(){let _this=this;let coldsdef=[];let numorder=[];
		if(this.xobj){this.xobj.destroy();}
let cnum=this.source.OH.length;
for(let c=0;c<cnum;c++){numorder.push(numorder.length);
	coldsdef.push({"orderDataType":"unified-ordering","type":'num-fmt',visible:true});
	coldsdef.push({"orderDataType":"tops-ordering",type:'string',visible:true});
	coldsdef.push({"orderDataType":"string-ordering",type:'string',visible:true});
}
let FO=this.source.get_formatted();
		let TABLE_OPTIONS={data:FO,"paging":false,dom:'Birtp',colReorder:true,columns:coldsdef,
			colReorder:{order:numorder},"language":it3.data.T.table_language,
		...
		if(SORTCOL!='nonono'){TABLE_OPTIONS.order=[[SORTCOL,'asc']];}
		this.xobj=$(t).DataTable(TABLE_OPTIONS);
	},
	update:function(){
		redraw();
	},
	...
	footer_recalc:function(api,row,data,start,end,display){
		
	}
};
Object.setPrototypeOf(GraphTable.prototype,GraphBase.prototype);
//(function(exports){exports.ReportData=ReportData;})(typeof exports === 'undefined'? this['ReportData']={}:exports);
(function(exports){exports.GraphTable=GraphTable;})(typeof exports === 'undefined'? (ISNODE?this['GraphTable']={}:this['it4']['data']['GraphTable']=GraphTable):exports);
...
_as_table:function(tgt,_revertdata,_redraw,_next,ui){let HH='';let FF='';let ts='';let coldsdef=[];let _this=this;
		let numorder=[];let 
		=[];
		if(!_redraw){nameorder=JSON.parse(JSON.stringify(this.source.OH));
			let b=it3.ins(this.element.parentElement,'button',['class','chart_times fa fa-download sel-ctr','style','top:40px;']);b.addEventListener('click',(ev)=>{_this.download_table();});
		}
		...
		for(let cidx=0;cidx<this.source.OH.length;cidx++){
			...
			let cname=this.source.OH[cidx];
			let cnameIDX=this.source.out_adapter[cname];
			if(this.source.oo.def.cols[cnameIDX].groupable){
				if(this.source.oo.def.cols[cnameIDX].gmode=='select'){HH=HH+'<th name="'+this.source.OH[cidx]+'"><select class="tablehfilter"><option value="all">'+this.source.OH[cidx]+'</option></select></th>';}
				else if(this.source.oo.def.cols[cnameIDX].gmode=='text'){HH=HH+'<th name="'+this.source.OH[cidx]+'"><input type="text" class="tablehfilter"/></th>';}
		...
		for(let h=0;h<this.source.OH.length;h++){
			...
			if(this.source.oo.def.cols[cnamesmallIDX].groupable){let hh=nameorder.indexOf(this.source.OH[h]);
				...
				for(let x=0;x<Fdata.length;x++){
					...
					let mrkStr='';if(!mrk){mrkStr=Fdata[x][mrkidx]}else{mrkStr=mrk;}
					if(fixfn){Fdata[x][h]=fixfn(Fdata[x][h],mrkStr);}
